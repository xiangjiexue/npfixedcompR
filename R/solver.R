...
solvegradientsingled1 = function(x, mu0, pi0, lower = min(x$v), upper = max(x$v), tol = 1e-6){
  ...
  a = lower; b = upper
  f1 = x$gradientfunction(mu = c(a, b), mu0 = mu0, pi0 = pi0, order = c(0, 1, 0))$d1
  ...
  repeat{
    ...
    c1 = (a + b) / 2
    fc[!index] = x$gradientfunction(c1[!index], mu0, pi0, order = c(0, 1, 0))$d1
    ...
    fs[!index] = x$gradientfunction(s[!index], mu0, pi0, order = c(0, 1, 0))$d1
...
solvegradientmultipled1 = function(x, mu0, pi0, points, tol = 1e-6){
  pointsval = x$gradientfunction(mu = points, mu0 = mu0, pi0 = pi0, order = c(0, 1, 0))$d1
  ...
  if (length(index) >= 1){
    ...
    r = c(points[1], points[length(points)])[x$gradientfunction(mu = c(points[1], points[length(points)]), mu0 = mu0, pi0 = pi0, order = c(1, 0, 0))$d0 < 0]
  }
  r
}
...
solvegradientsingled0 = function(x, mu0, pi0, lower = min(x$v), upper = max(x$v), tol = 1e-6){
  optimise(function(mu_){
    x$gradientfunction(mu = mu_, mu0 = mu0, pi0 = pi0, order = c(1, 0, 0))$d0
  }, lower = lower, upper = upper, tol = tol)$minimum
}

solvegradientmultipled0 = function(x, mu0, pi0, points, tol = 1e-6){
  pointsval = diff(x$gradientfunction(mu = points, mu0 = mu0, pi0 = pi0, order = c(1, 0, 0))$d0)
  index = seq(1, by = 1, length = length(points) - 1)[pointsval[-length(pointsval)] < 0 & pointsval[-1] > 0]
  if (length(index) >= 1){
    r = sapply(index, function(ddd){
...
solvegradientsingled2 = function(x, mu0, pi0, lower = min(x$v), upper = max(x$v), tol = 1e-6){
  ...
  r = x$gradientfunction(init, mu0, pi0, order = c(0, 1, 1))
  d1 = r$d1
  d2 = r$d2
  ...
  repeat{
    ...
    if (sum(mflag) > 0){
      s = (pos + neg) / 2
      d[!index & mflag] = x$gradientfunction(s[!index & mflag], mu0, pi0, order = c(0, 1, 0))$d1
    ...
    r = x$gradientfunction(init[!index], mu0, pi0, order = c(0, 1, 1))
    d1[!index] = r$d1
    d2[!index] = r$d2
  }
...
solvegradientmultiple = function(x, mu0, pi0, points, tol = 1e-6, method = "auto"){
  ...
  if (missing(points)){
    points = x$printgridpoints()
  }
  switch(x$printmethodflag(),
