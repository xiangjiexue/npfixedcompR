...
makeobject.npnormcvm = function(v, mu0, pi0, beta){
  if (class(v) == "npnormcvm"){
    ...
    x$precompute = seq(from = 0.5 / length(x$v), to = 1 - 0.5 / length(x$v), length = length(x$v)) -
      pnpnorm(x$v, mu0 = x$mu0, pi0 = x$pi0, sd = x$beta)
  }
...
gradientfunction.npnormcvm = function(x, mu, mu0, pi0, order = c(1, 0, 0)){
  ...
  if (order[1] == 1){
    temp = pnorm(x$v, mean = murep, sd = x$beta) * sum(pi0)
    ans$d0 = .colSums((temp - flexden) * fullden, m = length(x$v), n = length(mu))
  }
  if (any(order[2:3] == 1)){
    temp = dnorm(x$v, mean = murep, sd = x$beta) * fullden
  }
  if (order[2] == 1){
    ans$d1 = .colSums(temp, m = length(x$v), n = length(mu)) * -2 * sum(pi0)
  }
  if (order[3] == 1){
    ans$d2 = .colSums(temp * (x$v - murep), m = length(x$v), n = length(mu)) * -2 * sum(pi0)
  }
...
computemixdist.npnormcvm = function(x, mix = NULL, tol = 1e-6, maxiter = 100, verbose = FALSE){
  ...
  x$fn = function(mu0, pi0) pnpnorm(x$v, mu0 = mu0, pi0 = pi0, sd = x$beta)
...
estpi0d.npnormcvm = function(x, mu0, pi0){
  ans = vector("list", 2)
  names(ans) = c("d2", "d3")
  pn = pnpnorm(x$v, mu0 = mu0, pi0 = pi0, sd = x$beta)
  S = pnorm(x$v, sd = x$beta) - pn
  ans$d2 = 2 * sum(S * (pn - (2 * (1 : length(x$v)) - 1) / 2 / length(x$v))); ans$d3 = 2 * sum(S^2)
  ans
}
