#' @rdname makeobject
#' @export
makeobject.npnormcvmw = function(v, mu0, pi0, beta, order = -4){
  if (class(v) == "npnormcvmw"){
    ...
    x$precompute = cumsum(x$w) / sum(x$w) - 0.5 / sum(x$w) -
      pnpdiscnorm(x$v, mu0 = x$mu0, pi0 = x$pi0, sd = x$beta, h = x$h)
  }
...
lossfunction.npnormcvmw = function(x, mu0, pi0){
  sum((pnpdiscnorm(x$v, mu0 = mu0, pi0 = pi0, sd = x$beta, h = x$h) - x$precompute)^2 * x$w)
}
...
computemixdist.npnormcvmw = function(x, mix = NULL, tol = 1e-6, maxiter = 100, verbose = FALSE){
  ...
  x$fn = function(mu0, pi0) pnpdiscnorm(x$v, mu0 = mu0, pi0 = pi0, sd = x$beta, h = x$h)
  ...
  repeat{
    ...
    S = pdiscnorm(x$v, mean = rep(mu0new, rep(length(x$v), length(mu0new))), sd = x$beta, h = x$h)
...
estpi0d.npnormcvmw = function(x, mu0, pi0){
  ans = vector("list", 2)
  names(ans) = c("d2", "d3")
  pn = pnpdiscnorm(x$v, mu0 = mu0, pi0 = pi0, sd = x$beta, h = x$h)
  cdf = cumsum(x$w)
  S = pdiscnorm(x$v, sd = x$beta, h = x$h) - pn
  ans$d2 = 2 * sum(S * (pn - (2 * cdf - 1) / 2 / sum(x$w)) * x$w); ans$d3 = 2 * sum(S^2 * x$w)
  ans
}
