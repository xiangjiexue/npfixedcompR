...
gradientfunction.npnormcvmw = function(x, mu, mu0, pi0, order = c(1, 0, 0)){
  ...
  if (order[1] == 1){
    temp = pdiscnorm(x$v, mean = rep(mu, rep(length(x$v), length(mu))), sd = x$beta, h = x$h) * sum(pi0)
    ans$d0 = .colSums((temp - flexden) * fullden * x$w, m = length(x$v), n = length(mu))
  }
  if (any(order[2:3] == 1)){
    temp = dnorm(x$v + x$h, rep(mu, rep(length(x$v), length(mu))), sd = x$beta) * fullden * x$w
  }
  if (order[2] == 1){
