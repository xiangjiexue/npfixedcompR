...
gradientfunction.npnormad = function(x, mu, mu0, pi0, order = c(1, 0, 0)){
  ...
  if (order[1] == 1){
    ...
    ans$d0 = .colSums(pnorm(murep, sd = x$beta) * x$a1 / fullden +
                        pnorm(murep, sd = x$beta, lower.tail = FALSE) * x$a2 / fullden2,
                      m = length(x$v), n = length(mu)) * -sum(pi0) + addconst
  }
  if (any(order[2:3] == 1)){
    temp = dnorm(murep, sd = x$beta) * (x$a1 / fullden - x$a2 / fullden2)
  }
  if (order[2] == 1){
    ans$d1 = .colSums(temp, m = length(x$v), n = length(mu)) * sum(pi0)
  }
  if (order[3] == 1){
    ans$d2 = .colSums(murep * temp, m = length(x$v), n = length(mu)) * sum(pi0) / x$beta^2
  }
